...
void 				findTopWins(uint32_t readLen, SeedList *seeds, int isRev, int readIdx, int id);
...
void* mapSeq(void *idp)
{
	...
	while ((t=pf_getNextRead())<_pf_seqListSize)
	{
        ...
        std::sort(_pf_seedsForward[id].list, _pf_seedsForward[id].list + _pf_seedsForward[id].num, compareSeed);
        std::sort(_pf_seedsReverse[id].list, _pf_seedsReverse[id].list + _pf_seedsReverse[id].num, compareSeed);
        ...
        DEBUG({
	        ...
	        char *chrName_log;
            int32_t chrLen_log;
            seeds_log = _pf_seedsForward + id;
            fprintf(stderr, "\tfanchor\tnum: %u\n", seeds_log->num);
            for(ilog = 0; ilog < seeds_log->num; ilog++)
            {
                bwt_get_intv_info(seeds_log->list[ilog].tPos, seeds_log->list[ilog].tPos + seeds_log->list[ilog].len - 1, &chrName_log, &chrLen_log, &chrBeg_log, &chrEnd_log);
                fprintf(stderr, "\tfanchor\tread\t%u\t%u\ttarget\t%s\t%u\t%u\tlen\t%u\n", 
                    seeds_log->list[ilog].qPos, seeds_log->list[ilog].qPos + seeds_log->list[ilog].len - 1,
                    // seeds_log->list[ilog].tPos, seeds_log->list[ilog].tPos + seeds_log->list[ilog].len - 1,
                    chrName_log, chrBeg_log, chrEnd_log, 
                    seeds_log->list[ilog].len);
            }
        });
        DEBUG({
            SeedList *seeds_log;
	        ...
	        char *chrName_log;
            int32_t chrLen_log;
            seeds_log = _pf_seedsReverse + id;
            fprintf(stderr, "\tranchor\tnum: %u\n", seeds_log->num);
            for(ilog = 0; ilog < seeds_log->num; ilog++)
            {
                bwt_get_intv_info(seeds_log->list[ilog].tPos, seeds_log->list[ilog].tPos + seeds_log->list[ilog].len - 1, &chrName_log, &chrLen_log, &chrBeg_log, &chrEnd_log);
                fprintf(stderr, "\tranchor\tread\t%u\t%u\ttarget\t%s\t%u\t%u\tlen\t%u\n", 
                    seeds_log->list[ilog].qPos, seeds_log->list[ilog].qPos + seeds_log->list[ilog].len - 1,
                    // seeds_log->list[ilog].tPos, seeds_log->list[ilog].tPos + seeds_log->list[ilog].len - 1,
                    chrName_log, chrBeg_log, chrEnd_log, 
                    seeds_log->list[ilog].len);
        ...
        // fill in the _pf_topWins list
        for(i = 0; i < _pf_maxWin; i++)
        {
            _pf_topWins[id].list[i].tStart = 0;
            _pf_topWins[id].list[i].tEnd = 0;
            _pf_topWins[id].list[i].score = -1;
            _pf_topWins[id].list[i].isReverse = 0;
            std::push_heap(_pf_topWins[id].list, _pf_topWins[id].list + i + 1, compareWin);
        }
		_pf_topWins[id].num = _pf_maxWin;
...
void findTopWins(uint32_t readLen, SeedList *seeds, int isRev, int readIdx, int id)
{
    ...
    // for(int i = 0; i < seeds->num; i++)
    // {
    //     fprintf(stderr, "\tseed\t%u\t%u\t%u\t%u\n", seeds->list[i].qPos, seeds->list[i].qPos + seeds->list[i].len - 1, 
    //             seeds->list[i].tPos, seeds->list[i].tPos + seeds->list[i].len - 1);
    // }

    // fprintf(stderr, "======================\n");

    while(wEnd < seeds->num)
    {
        wMid = wEnd;
        // fprintf(stderr, "\t### %u %u %u\n", seeds->list[wBeg].tPos, seeds->list[wMid].tPos, seeds->list[wEnd].tPos);
        while(wEnd < seeds->num && seeds->list[wEnd].tPos < seeds->list[wMid].tPos + wLen)
        {
            wCnt++;
            wScr += 1 + (seeds->list[wEnd].len - WINDOW_SIZE);
            wEnd++;
        }
        // fprintf(stderr, "\t### %u %u %u\n", seeds->list[wBeg].tPos, seeds->list[wMid].tPos, seeds->list[wEnd].tPos);
        while(seeds->list[wBeg].tPos + wLen < seeds->list[wMid].tPos)
        {
            wCnt--;
            wScr -= 1 + (seeds->list[wBeg].len - WINDOW_SIZE);
            wBeg++;
        }
        // fprintf(stderr, "\t### %u %u %u\n", seeds->list[wBeg].tPos, seeds->list[wMid].tPos, seeds->list[wEnd].tPos);
        // fprintf(stderr, "\t window\t%d %d\t%d, %d\t%u, %u\n", wCnt, wScr, wBeg, wEnd-1, seeds->list[wBeg].tPos, seeds->list[wEnd-1].tPos + seeds->list[wEnd-1].len - 1);

        // if( wScr > _pf_topWins[id].list[0].score) // if the score is higher than the current smallest score, replace it with that
        // {
        ...
        //     _pf_topWins[id].list[_pf_topWins[id].num-1].tStart = seeds->list[wBeg].tPos;
        //     _pf_topWins[id].list[_pf_topWins[id].num-1].tEnd = seeds->list[wEnd-1].tPos + seeds->list[wEnd-1].len - 1;
        //     _pf_topWins[id].list[_pf_topWins[id].num-1].score = wScr;
        ...
        if(wCnt > 1)
            tmpScore = calcChainScore(readLen, seeds->list[wBeg].tPos, seeds->list[wEnd-1].tPos + seeds->list[wEnd-1].len - 1, isRev, id);
        else
            tmpScore = wScr;
...
void alignWin(Win_t &win, char *query, char *query_rev, uint32_t rLen, Sam_t &map, int id)
{
	...
	if(win.isReverse)
	{
        ...
        DEBUG({
	        ...
	        char *chrName_log;
            int32_t chrLen_log;
            seeds_log = _pf_seedsSelected;
            fprintf(stderr, "\tsanchor\tnum: %u\n", seeds_log->num);
            for(ilog = 0; ilog < seeds_log->num; ilog++)
            {
                bwt_get_intv_info(seeds_log->list[ilog].tPos, seeds_log->list[ilog].tPos + seeds_log->list[ilog].len - 1, &chrName_log, &chrLen_log, &chrBeg_log, &chrEnd_log);
                fprintf(stderr, "\tsanchor\tread\t%u\t%u\ttarget\t%s\t%u\t%u\tlen\t%u\n", 
                    seeds_log->list[ilog].qPos, seeds_log->list[ilog].qPos + seeds_log->list[ilog].len - 1,
                    // seeds_log->list[ilog].tPos, seeds_log->list[ilog].tPos + seeds_log->list[ilog].len - 1,
                    chrName_log, chrBeg_log, chrEnd_log, 
                    seeds_log->list[ilog].len);
...
void alignChain_edlib(Chain_t &chain, char *query, int32_t readLen, Sam_t &map)
{
    ...
    DEBUG({
        fprintf(stderr, "\talign\tleft\t%s\n", std::string(edCigar->begin(), edCigar->end()).c_str());
    });
	...
	for(i=0; i<chain.chainLen-1; i++)
	{
        ...
        DEBUG({
            fprintf(stderr, "\talign coord\t%d\t%d\n", readAlnStart, readAlnEnd);
            fprintf(stderr, "\talign coord\t%d\t%d\n", refAlnStart, refAlnEnd);
            fprintf(stderr, "\talign coord\t%d\t%d\n", readAlnLen, refAlnLen);
        });
        ...
        DEBUG({
            fprintf(stderr, "\talign\t(%d, %d)\t%s\n", i, i+1, std::string(edCigar->begin(), edCigar->end()).c_str());
        });
	}
	// // last seed
	// fprintf(stderr, "\tseed\t%u\t%u\t%u\t%u\t%u\n", chain.seeds[i].qPos, chain.seeds[i].qPos + chain.seeds[i].len - 1, 
